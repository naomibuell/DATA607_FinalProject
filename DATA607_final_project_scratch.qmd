...
nyt_movies_raw <- get_movies(base_url, api_key, filter, num_pages, num_seconds)
```
...
```{r}
reviewScrape = function(uri, output){
  success <- FALSE
    
  while (success == FALSE) {
    tryCatch({
      htmlText <- read_html(uri["web_url"]) %>%
...
```{r}
reviews <- apply(merged_no_dupl,1,reviewScrape  )
